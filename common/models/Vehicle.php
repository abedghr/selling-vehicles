...
class Vehicle extends \common\models\BaseModels\Vehicle
{
    ...
    public $imageFile;
    public $_new_vehicle, $_used_vehicle;
    public $multiple_tbl = [
        '_new_vehicle' => [
            'scenario' => ['create_new_vehicle', 'update_new_vehicle'],
            'model' => '\common\models\NewVehicle',
            'form_name' => 'NewVehicle',
        ],
        '_used_vehicle' => [
            'scenario' => ['create_used_vehicle', 'update_used_vehicle'],
            'model' => '\common\models\UsedVehicle',
            'form_name' => 'UsedVehicle',
        ]
    ];
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['imageFile'], 'file', 'maxFiles' => 1, 'extensions' => 'png, jpg, jpeg', 'skipOnEmpty' => true, 'on' => self::SCENARIO_UPDATE],
            [['imageFile'], 'file', 'maxFiles' => 1, 'extensions' => 'png, jpg, jpeg', 'skipOnEmpty' => false, 'on' => self::SCENARIO_CREATE]
...
//                if (!$single_image->saveAs('uploads/vehicle/' . time() . '_' . $single_image)) {
    ...
    public function save($runValidation = true, $attributeNames = null)
    {
        ...
        if($this->type == self::TYPE_NEW) {
            $this->_new_vehicle->vehicle_id = $this->id;
            if(!$this->_new_vehicle->save()) {
                $transaction->rollBack();
                return false;
            }
        }

        if($this->type == self::TYPE_USED) {
            $this->_used_vehicle->vehicle_id = $this->id;
            if(!$this->_used_vehicle->save()) {
                $transaction->rollBack();
                return false;
            }
        }

        if(!$this->MultiUploadImages($media)) {
            $transaction->rollBack();
            return false;
        }
